...
Процедура РасчитатьПроцентНесоответсвия()
	...
	ПроцентСоответсвия = 100 - Объект.ПроцентНессответсвий;
	
	Если ПроцентСоответсвия >= 95 Тогда 
		Объект.Оценка = 5;
	ИначеЕсли ПроцентСоответсвия >= 90 И ПроцентСоответсвия <= 94 Тогда
		Объект.Оценка = 4;
	ИначеЕсли ПроцентСоответсвия >= 85 И ПроцентСоответсвия <= 89 Тогда
		Объект.Оценка = 3;
	ИначеЕсли ПроцентСоответсвия >= 80 И ПроцентСоответсвия <= 84 Тогда
		Объект.Оценка = 2;
	ИначеЕсли ПроцентСоответсвия >= 75 И ПроцентСоответсвия <= 79 Тогда
		Объект.Оценка = 1;
	Иначе
		Объект.Оценка = 0;
	КонецЕсли;
