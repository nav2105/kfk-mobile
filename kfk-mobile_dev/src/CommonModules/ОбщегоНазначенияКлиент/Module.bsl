...
Функция КаталогФото(ЗаполнитьКонстанту = Ложь) Экспорт
	
	ПутьККаталогу = КаталогДокументов() + "/kfk_photo";
	КаталогФото = Новый Файл(ПутьККаталогу);
	Если Не КаталогФото.Существует() Тогда 
		СоздатьКаталог(ПутьККаталогу);
		ОбщегоНазначенияВызовСервера.УстановитьЗначениеКонстанты("КаталогФото", ПутьККаталогу);
		ЗаполнитьКонстанту = Ложь;
	КонецЕсли;
	
	Если ЗаполнитьКонстанту Тогда
		ОбщегоНазначенияВызовСервера.УстановитьЗначениеКонстанты("КаталогФото", ПутьККаталогу);
	КонецЕсли;		
