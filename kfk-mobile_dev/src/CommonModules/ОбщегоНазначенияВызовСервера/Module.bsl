...
Процедура ЗапуститьФоновоеУдалениеСтарыхФото() Экспорт
	ФоновыеЗадания.Выполнить("ОбщегоНазначенияВызовСервера.УдалитьСтарыеФото", , "ФоновоеУдалениеСтарыхФото", "ФоновоеУдалениеСтарыхФото");
КонецПроцедуры
...
Процедура УдалитьСтарыеФото() Экспорт
	...
	МассивФайловДляУдаления = ПолучитьМассивСтарыхФото();
		
	Для Каждого АдресНаДиске из  МассивФайловДляУдаления Цикл
		Попытка 
			УдалитьФайлы(АдресНаДиске);
		Исключение
		КонецПопытки;
	КонецЦикла;		
...
Функция ПолучитьМассивСтарыхФото() Экспорт
	...
	Если КоличествоДнейХраненияФото = 0 Тогда
		Возврат МассивФайловДляУдаления;
	КонецЕсли;
	
	ДатаХранения = НачалоДня(ТекущаяДата()) - (КоличествоДнейХраненияФото * 86400); 
	МассивФайлов = НайтиФайлы(КаталогФото, "*.jpg", Ложь);
	
	Для Каждого ФайлФото из МассивФайлов Цикл
		Если ФайлФото.ПолучитьВремяИзменения() < ДатаХранения Тогда
			МассивФайловДляУдаления.Добавить(ФайлФото.ПолноеИмя)
		КонецЕсли;		
	КонецЦикла; 

	Запрос = Новый Запрос("ВЫБРАТЬ
	|	ОчередьФотоНаСервер.АдресНаДиске
	|ИЗ
	|	РегистрСведений.ОчередьФотоНаСервер КАК ОчередьФотоНаСервер
	|ГДЕ
	|	Подстрока(ОчередьФотоНаСервер.АдресНаДиске, 0, 400) В (&МассивАдресов)");

	Запрос.УстановитьПараметр("МассивАдресов", МассивФайловДляУдаления);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		МассивФайловДляУдаления.Удалить(МассивФайловДляУдаления.Найти(Выборка.АдресНаДиске));		
	КонецЦикла;	
